...
const TrustedBySection = () => {
    return (
        <div className='h-1/2 px-2 py-2 my-4 md:px-2 md:py-12 container mx-auto max-w-7xl' >
            ...
            <ul className='grid grid-cols-2 grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4'>
                {trustedBy.map(company => {
                    ...
                    return <li key={id}  className='p-4  cursor-pointer mx-auto ' data-aos="fade-down"><img className='opacity-40 hover:opacity-100 transition' src={image} alt={id} /></li>
                })}
            </ul>
        </div>
    )
